...
//                      Collectors.toMap(Meal::getDate, Meal::getCalories, Integer::sum)
    ...
    public static List<MealTo> getListMeals(int caloriesPerDay) {
        Map<LocalDate, Integer> mapCaloriesPerDay = meals.stream()
                .collect(Collectors.groupingBy(Meal::getDate, Collectors.summingInt(Meal::getCalories)));

        return meals.stream()
                .map(item -> new MealTo(item.getId(), item.getDateTime(), item.getDescription(), item.getCalories(), mapCaloriesPerDay.get(item.getDate()) > caloriesPerDay))
                .collect(Collectors.toList());
    }
}